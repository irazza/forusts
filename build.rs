...
fn main() {
    ...
    let filtered_paths = paths.filter_map(|path| {
    ...
    for path in filtered_paths {
        ...
        let temp_path = PathBuf::from(env::var("OUT_DIR").unwrap()).join(format!("bindings_{}.rs", name));
        bindings
    ...
    for (name, path) in header_paths {
        bindings.push_str(&format!("pub mod {}{{include ! {{ \"{}\" }} }}\n", name, path.display()));
    }
    fs::write(PathBuf::from(env::var("OUT_DIR").unwrap()).join("bindings.rs"), bindings).unwrap();
