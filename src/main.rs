...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for (i, path) in datasets.iter().enumerate() {
        ...
        .unwrap();
        let mut ds_train = ds_train.iter().map(|s| Sample{ features: Arc::new(zscore(&s.features)), target: s.target}).collect::<Vec<_>>();
        ...
        .unwrap();
        let ds_test = ds_test.iter().map(|s| Sample{ features: Arc::new(zscore(&s.features)), target: s.target}).collect::<Vec<_>>();
    ...
    println!("Mean: {}", predictions.iter().flatten().sum::<f64>() / (n_repetitions * datasets.len()) as f64);
    // add first column with dataset names
    let predictions = datasets
        .iter().zip(predictions)
        .map(|(d, p)| {
        ...
        .collect::<Vec<Vec<_>>>();
    write_csv("cisof_H.csv", predictions, None);
    Ok(())
}
