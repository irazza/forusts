...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for path in &datasets {
        ...
        for _i in 0..n_repetitions {
            ...
            let mut anomaly_scores = clf.score_samples(&ds_train);
            anomaly_scores.sort_by(|a, b| a.partial_cmp(b).unwrap());

            let score = anomaly_scores[anomaly_scores.len() -1] - anomaly_scores[0];
            if score > best_score {
                best_score = score;
                best_model = Some(clf);
            }
        ...
        let forest_accuracy = accuracy_score(&model.predict(&ds_test), &y_true);
        ...
        println!(
            "\tForest: {}, Breiman: {}, Ancestor: {}, Zhu: {}",
            forest_accuracy, accuracy_breiman, accuracy_ancestor, accuracy_zhu);
    }
