...
mod tests {
    ...
    //         println!("Dataset: {}", path.path().file_stem().unwrap().to_string_lossy());
    ...
    //             }
    //         }
    //     let folder = format!("results/{}", path.path().file_stem().unwrap().to_string_lossy());
    //     if !std::path::Path::new(&folder).exists() {
    //         fs::create_dir_all(&folder).unwrap();
    //     }
    //     let mut wtr = csv::Writer::from_path(format!("results/{}/anomaly_scores.csv", path.path().file_stem().unwrap().to_string_lossy())).unwrap();
    //     for scores in anomaly_scores {
    //         wtr.write_record(scores.iter().map(|x| x.to_string()))
    //             .unwrap();
    //     }
    //     wtr.flush().unwrap();
    //     let mut wtr = csv::Writer::from_path(format!("results/{}/auc_scores.csv", path.path().file_stem().unwrap().to_string_lossy())).unwrap();
    //     wtr.write_record(auc_scores.iter().map(|x| x.to_string()))
    //         .unwrap();
    //     wtr.flush().unwrap();
    //     let mut wtr = csv::Writer::from_path(format!("results/{}/precision_at_10.csv", path.path().file_stem().unwrap().to_string_lossy())).unwrap();
