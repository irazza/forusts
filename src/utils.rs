...
pub fn matthews_corrcoef(y_pred: &Vec<usize>, y_true: &Vec<usize>) -> f64
{
    ...
    let p_sum: Vec<usize> = (0..confusion_matrix[0].len()).into_iter().map(|c| {
        confusion_matrix.iter().map(|r| r[c]).sum()
    }).collect();
    let n_correct = confusion_matrix.iter().enumerate().map(|(i, r)| r[i]).sum::<usize>();
    let n_samples = p_sum.iter().sum::<usize>();
    let cov_ytyp = (n_correct * n_samples) as f64 - t_sum.iter().zip(p_sum.iter()).map(|(a, b)| a * b).sum::<usize>() as f64;
    let cov_ypyp = (n_samples*n_samples) as f64 - p_sum.iter().zip(p_sum.iter()).map(|(a, b)| a * b).sum::<usize>() as f64;
    let cov_ytyt = (n_samples*n_samples) as f64 - t_sum.iter().zip(t_sum.iter()).map(|(a, b)| a * b).sum::<usize>() as f64;
